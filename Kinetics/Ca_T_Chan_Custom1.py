...
def Ca_T_Chan(name):
    ...
    htau[htau<hmin]=hmin
    YtblA = np.zeros([Vdivs,1])
    YtblB = np.zeros([Vdivs,1])
    for i in np.arange(1):
        YtblA[:,i] = hinf/htau
        YtblB[:,i] = 1/htau
        print(i, end='\r')
    ygate.tableA = YtblA*1e3
    ygate.tableB = YtblB*1e3

    # ezfrt  = np.exp(z*v*F/R/T)
    # ZtblA = np.zeros([Vdivs,Cadivs])
    # ZtblB = np.zeros([Vdivs,Cadivs])
    # for i in np.arange(Cadivs):
    #     ZtblA[:,i] = v*(ca[i]/cao*ezfrt-1)/(ezfrt-1)/(v-ECa)
    #     print(i, end='\r')
    # ZtblB = ZtblA*0 +1
    # zgate.tableA = ZtblA
    # zgate.tableB = ZtblB
    ...
    ZtblA = np.transpose(caezfrt*caezfrt_ex)
    ZtblB = ZtblA*0 +1
    zgate.tableA = ZtblA
    zgate.tableB = ZtblB

    # np.savez('Ca_T_Chan_Custom1_tbls.npz', XtblA=XtblA, XtblB=XtblB, YtblA=YtblA, YtblB=YtblB, ZtblA=ZtblA, ZtblB=ZtblB)
    # tbls = np.load('Ca_T_Chan_Custom1_tbls.npz')
    # xgate.tableA = tbls['XtblA']
    # xgate.tableB = tbls['XtblB']
    # ygate.tableA = tbls['YtblA']
    # ygate.tableB = tbls['YtblB']
    # zgate.tableA = tbls['ZtblA']
    # zgate.tableB = tbls['ZtblB']
