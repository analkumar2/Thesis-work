...
def expscore(cellpath, meanfeature, stdfeature, expfeature=None):
    if expfeature==None:
        expfeature = expfeatures(cellpath,stim_start=1,stim_end=1.5)
    if expfeature==False:
        return False
    def removestrcol(dictt):
        dicttk = dictt.keys()
        for keyy in dicttk:
            if isinstance(dictt[keyy],str):
                dictt.pop(keyy)
    [removestrcol(dictt) for dictt in [meanfeature, stdfeature, expfeature]]

    score = {}
    for keyy in meanfeature.keys():
        score[keyy] = (expfeature[keyy]-meanfeature[keyy])/stdfeature[keyy]
