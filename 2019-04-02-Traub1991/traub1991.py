...
#exec(open('/mnt/c/Analkumar2/Study/Biology/Neuroscience/2018 - 23 PhD Thesis/Upi rotation/replicating traub 1991/traub1991.py').read())
...
# EREST = 0
F = 96485.3329
morphfile = '/mnt/c/Analkumar2/Study/Biology/Neuroscience/2018 - 23 PhD Thesis/Upi rotation/replicating traub 1991/Compartments.swc'
ChP = '/mnt/c/Analkumar2/Study/Biology/Neuroscience/2018 - 23 PhD Thesis/Upi rotation/replicating traub 1991/Channelprotos'
...
rdes = rd.rdesigneur(
    ...
    chanProto = [
        [ChP+'.NaChan()', 'Na_Chan'],
        [ChP+'.KdrChan()', 'Kdr_Chan'],
        [ChP+'.KaChan()', 'Ka_Chan'],
        [ChP+'.CaConc()', 'Ca_conc'],
        [ChP+'.CaChan()', 'Ca_Chan'],        
        [ChP+'.KahpChan()', 'Kahp_Chan'],
        [ChP+'.KcChan()', 'Kc_Chan'],
    ],
    ...
    chanDistrib = [
        ...
        # ['Kahp_Chan', 'soma', 'Gbar', '0.4'], #reduced 20 times
        ['Kc_Chan', 'soma', 'Gbar', '100'],
        # ['Kc_Chan', 'soma', 'Gbar', '5'], #reduced 20 times
        ...
        # ['Kahp_Chan', 'apical#', 'Gbar', '(p<=1080e-6) ? 0.4 : 0'], #reduced 20 times
        ['Kc_Chan', 'apical#', 'Gbar', '(p<=120e-6) ? 200 : ((p>=120e-6 && p<=240e-6) ? 50 : ((p>=240e-6 && p<=840e-6) ? 100 : ((p>=840e-6 && p<=1080e-6) ? 50 : 0)))'],
        # ['Kc_Chan', 'apical#', 'Gbar', '(p<=120e-6) ? 10 : ((p>=120e-6 && p<=240e-6) ? 50 : ((p>=240e-6 && p<=840e-6) ? 100 : ((p>=840e-6 && p<=1080e-6) ? 50 : 0)))'],
        ...
        # ['Kahp_Chan', 'dend#', 'Gbar', '(p<=770e-6) ? 0.4 : 0'], #reduced 20 times
        ['Kc_Chan', 'dend#', 'Gbar', '(p<=110e-6) ? 200 : ((p>=110e-6 && p<=220e-6) ? 50 : ((p>=220e-6 && p<=550e-6) ? 100 : ((p>=550e-6 && p<=770e-6) ? 50 : 0)))'],
        # ['Kc_Chan', 'dend#', 'Gbar', '(p<=110e-6) ? 10 : ((p>=110e-6 && p<=220e-6) ? 50 : ((p>=220e-6 && p<=550e-6) ? 100 : ((p>=550e-6 && p<=770e-6) ? 50 : 0)))'],
    
    ],
        
    stimList = [
        # ['soma', '1', '.', 'inject', '(t>=3 && t<3.003) ? 3e-9 : ((t>=3.003 && t<=3.006) ? -5e-9 : 0)' ],
        ['soma', '1', '.', 'inject', '(t>=3.0 && t<=8.003) ? 3e-9 : 0'],
    ],
