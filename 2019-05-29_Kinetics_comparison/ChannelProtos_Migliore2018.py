...
def Na_Chan(name):
    ...
    alpv = 1/(1+np.exp((v*1e3-vvh-sh)/vvs))
    alps = np.exp(1.e-3*zetas*(v*1e3-vhalfs-sh)*9.648e4/(8.315*(273.16+celsius)))
    bets = np.exp(1.e-3*zetas*gms*(v*1e3-vhalfs-sh)*9.648e4/(8.315*(273.16+celsius)))
    qt=q10**((celsius-24)/10)
    a = [trap0(vm,tha+sh2,Ra,qa) for vm  in v]
    b = [trap0(-vm,-tha-sh2,Rb,qa) for vm in v]
    mtau = (1/(a+b))/qt
    mtau[mtau<mmin] = mmin
    minf = a/(a+b)

    a = [trap0(vm,thi1+sh2,Rd,qd) for vm in v]
    b = [trap0(-vm,-thi2-sh2,Rg,qg) for vm in v]
    htau =  (1/(a+b))/qt
    htau[htau<hmin) = hmin
