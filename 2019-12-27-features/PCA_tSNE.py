...
features_WT = pd.read_csv('features_ess_WT.csv',sep='\t', index_col=0)
features_KO = pd.read_csv('features_ess_KO.csv',sep='\t', index_col=0)
features = pd.concat([features_WT,features_KO], ignore_index=True)

y_train = np.concatenate([np.ones(27),np.zeros(17)])
X_train = features.drop(['Cell name'], axis=1)
