#exec(open('6compartment/channel_slider_traubsrikanth.py').read())
...
#Myown
ChPo = '6compartment/ChannelProtos_Sri2015_base'
ChP = '6compartment/ChannelProtos_Sri2015_base'
# ChP = '6compartment/ChannelProtos_Combe2018'
# exec(open('6compartment/feature_dict.py').read()) #Get the feature_range
F = 96485.3329
...
# with open('6compartment/cost0.pkl', 'rb') as f:
#     Em, RM, RA, CM, sm_diam, sm_len, \
#     Ca_Basal, Ca_tau, Ca_B, Na_Gbar, K_DR_Gbar, K_A_Gbar, \
#     K_M_Gbar, h_Gbar, Ca_T_Gbar, Ca_R_Gbar, \
#     Ca_L_Gbar, Ca_N_Gbar, K_SK_Gbar, K_BK_Gbar = pickle.load(f)
...
def makeModel(stim_start = preStimTime, stim_end = preStimTime+injectTime, curr = Injectcurr):
    rdes = rd.rdesigneur(
        ...
        cellProto = [['6compartment/Compartments.swc','elec']],
        chanProto = [
            [ChP+'.Na_Chan()', 'Na_chan'],
        ...
        stimList = [
            # ['soma', '1', '.', 'vclamp', '-0.065 + (t>1 && t<1.5) * 0.050' ],
...
def parameters():
    ...
    prms['RA'] = soma.Ra*soma.diameter**2/soma.length/4*np.pi
    prms['Em'] = soma.Em
...
def save_params(event):
    with open('6compartment/best_params.pkl', 'wb') as f:
