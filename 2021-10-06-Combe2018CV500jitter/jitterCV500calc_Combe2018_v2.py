...
def calcCVjit():
    ...
    for i in range(3):
        ...
        curr[int(stim_start*samprate):int(stim_end*samprate)] = 150e-12
    ...
    if pc.nhost()==1:
        for i in range(3):
            A.append(get_Vmvec([tI_list[i], II_list[i]]))
    else:            
        for i in range(3):
            pc.submit(get_Vmvec, [tI_list[i], II_list[i]])
        while pc.working():
            A.append(pc.pyret())
