...
def Modelfunc(runfor=2, stimul='Iclamp', Injectcurr=Injectcurr, gl=1/Parameters['RM'], Ca_concCaBasal=str(Parameters['Ca_concCaBasal']), Ca_conctau=str(Parameters['Ca_conctau']), Ca_L_changbar=str(Parameters['Ca_L_changbar']), Ca_N_changbar=str(Parameters['Ca_N_changbar']), Ca_T_changbar=str(Parameters['Ca_T_changbar']), h_changbar=str(Parameters['h_changbar']), K_A_changbar=str(Parameters['K_A_changbar']), K_BK_changbar=str(Parameters['K_BK_changbar']), K_D_changbar=str(Parameters['K_D_changbar']), K_DR_changbar=str(Parameters['K_DR_changbar']), K_M_changbar=str(Parameters['K_M_changbar']), K_SK_changbar=str(Parameters['K_SK_changbar']), Na_changbar=str(Parameters['Na_changbar']), Na_P_changbar=str(Parameters['Na_P_changbar'])):
    ...
    try:
        ...
        moose.delete('/library/K_A_chan')
        moose.delete('/library/K_DR_chan')
    ...
    rdes = rd.rdesigneur(
        ...
        chanProto = [
            ...
            ['K_A_Chan_(Migliore2018)_ghk.K_A_Chan()', 'K_A_chan'],
            ...
            ['Na_P_Chan_(Migliore2018).Na_P_Chan()', 'Na_P_chan'],
        ],
        passiveDistrib = [
        ...
        chanDistrib = [
            ...
            ['Na_P_chan', 'soma', 'Gbar', Na_P_changbar],
        ],
        stimList = [
    ...
    #########ca_conc to eca###########
    # eca_nernst = moose.Nernst('ca')
    # eca_nernst.tick = 6
    # eca_nernst.Cin = 0.05e-3
    # eca_nernst.Cout = 10
    # eca_nernst.valence = 2
    # eca_nernst.Temperature = 305
    ...
    # # moose.connect(Ca_conc, 'concOut', eca_nernst, 'ci')
    # moose.connect(eca_nernst, 'Eout', Ca_L_chan, 'setEk')
    # moose.connect(eca_nernst, 'Eout', Ca_N_chan, 'setEk')
    # moose.connect(eca_nernst, 'Eout', Ca_T_chan, 'setEk')

    eca_nernst = moose.Function('/model/ca')
    eca_nernst.c['R'] =  8.314
    eca_nernst.c['T'] = 305
    eca_nernst.c['z'] = 2
    eca_nernst.c['F'] = 96485
    eca_nernst.c['cout'] = 2
    eca_nernst.expr = 'R*T/z/F*ln(cout/y0)'
    eca_nernst.mode = 1
    eca_nernst.tick = 6
    ...
    Ca_L_chan = moose.element('/model/elec/soma/Ca_L_chan')
    Ca_conc = moose.element('/model/elec/soma/Ca_conc')
    moose.connect(eca_nernst, 'requestOut', Ca_conc, 'getCa')
    moose.connect(eca_nernst, 'valueOut', Ca_T_chan, 'setEk')
    moose.connect(eca_nernst, 'valueOut', Ca_L_chan, 'setEk')
    moose.connect(eca_nernst, 'valueOut', Ca_N_chan, 'setEk')
    ##################################
    ...
    Na_P_chan = moose.element('/model/elec/soma/Na_P_chan')
    ...
    Gk = Na_chan.Gk + Na_P_chan.Gk + K_SK_chan.Gk + K_M_chan.Gk + K_DR_chan.Gk + K_D_chan.Gk + K_BK_chan.Gk + K_A_chan.Gk + h_chan.Gk + Ca_T_chan.Gk + Ca_N_chan.Gk + Ca_L_chan.Gk #initial calculation of total active conductance
    Ginc = Gl+Gk
    ...
    if Ginc<Gin-1e-9 or Ginc>Gin+1e-9:
        if Gk<Gin:
            ...
            Gl = mingl*sm_area
            # Na_P_changbar = (Gin - Gl - (Gk - K_D_chan.Gk))/Pr['K_D_chan']
            K_D_chanGbar = (Gin - Gl - (Gk - K_D_chan.Gk))/Pr['K_D_chan']
            if K_D_chanGbar<0:
                K_D_chanGbar = 0
                Gk = -1*(0 - Na_chan.Gk - Na_P_chan.Gk - K_SK_chan.Gk - K_M_chan.Gk - K_DR_chan.Gk - K_BK_chan.Gk - K_A_chan.Gk - h_chan.Gk - Ca_T_chan.Gk - Ca_N_chan.Gk - Ca_L_chan.Gk)
                h_chanGbar = (Gin - Gl - (Gk - h_chan.Gk))/Pr['h_chan']
                if h_chanGbar<0:
                    h_chanGbar = 0
                    Gk = -1*(0 - Na_chan.Gk - Na_P_chan.Gk - K_SK_chan.Gk - K_M_chan.Gk - K_DR_chan.Gk - K_BK_chan.Gk - K_A_chan.Gk - Ca_T_chan.Gk - Ca_N_chan.Gk - Ca_L_chan.Gk)
                    Na_P_chanGbar = (Gin - Gl - (Gk - Na_P_chan.Gk))/Pr['Na_P_chan']
                    if Na_P_chanGbar<0:
                        Na_P_chanGbar = 0
                        Gk = -1*(0 - Na_chan.Gk - K_SK_chan.Gk - K_M_chan.Gk - K_DR_chan.Gk - K_BK_chan.Gk - K_A_chan.Gk - Ca_T_chan.Gk - Ca_N_chan.Gk - Ca_L_chan.Gk)
                        K_M_chanGbar = (Gin - Gl - (Gk - K_M_chan.Gk))/Pr['K_M_chan']
                        if K_M_chanGbar<0:
                            print('Recheck model. BK currents are too high')
                            plt.close()
                            sureshoterror
                        K_M_chan.Gbar = K_M_chanGbar
                    Na_P_chan.Gbar = Na_P_chanGbar
    ...
    Gk = Na_chan.Gk + Na_P_chan.Gk + K_SK_chan.Gk + K_M_chan.Gk + K_DR_chan.Gk + K_D_chan.Gk + K_BK_chan.Gk + K_A_chan.Gk + h_chan.Gk + Ca_T_chan.Gk + Ca_N_chan.Gk + Ca_L_chan.Gk #initial calculation of total active conductance
    Ginc = Gl+Gk
    Iactive = Na_chan.Ik + Na_P_chan.Ik + K_SK_chan.Ik + K_M_chan.Ik + K_DR_chan.Ik + K_D_chan.Ik + K_BK_chan.Ik + K_A_chan.Ik + h_chan.Ik + Ca_T_chan.Ik + Ca_N_chan.Ik + Ca_L_chan.Ik
    Em = -Iactive/Gl + Vrest
    ...
    Na_P_changbar=str(Na_P_chan.Gbar/sm_area)
    ...
    Parametersout['Na_P_changbar'] = Na_P_chan.Gbar/sm_area
