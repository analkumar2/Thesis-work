...
# temp: Only for use in this folder
...
def runModel(
    ...
    # AISelements = moose.le("/model/elec/axon_1_2") #where Ca_conc element is situated. In this case its everywhere but we plot soma's
    # Parameters = Parameterdict_parser(Parameterdict)
    ...
    for Ca_concelement in moose.wildcardFind("/model/elec/##[CLASS==ZombieCaConc]"):
        ...
        # Ca_concelement.B = Parameters["Ca_B"]*4/(np.pi*parrent.diameter*parrent.diameter*parrent.length)
    ...
    if "/model[0]/elec[0]/soma[0]/Ca_conc" in somaelements:
