...
def calcRinCin(modeldict, refreshKin=True, stim_start=1):
    ...
    def chargingm25(t, R1, R2, tau1, tau2):
        return E_rest - R1 * 25e-12 * (1 - np.exp(-t / tau1)) - R2 * 25e-12 * (1 - np.exp(-t / tau2))
    ...
    # plt.plot(np.linspace(0, 0.1, len(tempv)), tempv)
    # plt.plot(np.linspace(0, 0.1, len(tempv)), chargingm25(np.linspace(0, 0.1, len(tempv)), *RCfitted_chm25))
    # plt.show()
...
def ftscalc_helper(
    ...
    for I in [150e-12, 300e-12]:
        ...
        f2 = sciip.interp1d(vv[p1p-10:p1p+1], tt[p1p-10:p1p+1])
        f3 = sciip.interp1d(vv[p1p:p1p+30+1], tt[p1p:p1p+30+1])
        ...
        p1p = sweep_ext.spike_feature('peak_index')[-2]
        f2p = sciip.interp1d(Vgradp[p1p-10:p1p+1], tt[p1p-10:p1p+1])
        f3p = sciip.interp1d(tt[p1p-10:p1p+1], vv[p1p-10:p1p+1])
        Thresh_tp = f2p(25)
        ...
        Vgradp = np.gradient(vv, tt)
        p1p = sweep_ext.spike_feature('peak_index')[-2]
        Vgradmaxidxp = np.argmax(Vgradp[p1p-10:p1p+1])
        Up_tp = tt[Vgradmaxidxp+p1p-10]
        Up_vp = vv[Vgradmaxidxp+p1p-10]
        ...
        features[f"Upstroke_APp_value_{I}"]= Vgradp[Vgradmaxidxp+p1p-10]
        ...
        Vgradp = np.gradient(vv, tt)
        p1p = sweep_ext.spike_feature('peak_index')[-2]
        Vgradminidxp = np.argmin(Vgradp[p1p:p1p+30+1])
        ...
        features[f"Downstroke_APp_value_{I}"] = Vgradp[Vgradminidxp+p1p-10]
...
def modelfeatures(modeldict, stim_start=1, stim_end=1.5, apa=True, refreshKin=True):
    ...
    if isinstance(features, dict) and apa:
        tbAp, IbAp, Ca = mm.runModel(
            modeldict, vClamp="-0.055 + (t>1 && t<1.1)*-0.010 + (t>1.1 && t<1.9)*0.020", refreshKin=False
        )
        modeldict_temp = deepcopy(modeldict)
        modeldict_temp["Parameters"]["Channels"]["K_SK_Chan"]["Gbar"] = 1e-15
        taAp, IaAp, Ca = mm.runModel(
            modeldict_temp,
            vClamp="-0.055 + (t>1 && t<1.1)*-0.010 + (t>1.1 && t<1.9)*0.020", refreshKin=False
        )
        features["apaCurrm35"] = (
        ...
        tbAp, IbAp, Ca = mm.runModel(
            modeldict, vClamp="-0.055 + (t>1 && t<1.1)*-0.010 + (t>1.1 && t<1.9)*0.090", refreshKin=False
        )
        modeldict_temp = deepcopy(modeldict)
        modeldict_temp["Parameters"]["Channels"]["K_SK_Chan"]["Gbar"] = 1e-15
        taAp, IaAp, Ca = mm.runModel(
            modeldict_temp,
            vClamp="-0.055 + (t>1 && t<1.1)*-0.010 + (t>1.1 && t<1.9)*0.090", refreshKin=False
        )
        features["apaCurr35"] = (
            IbAp[np.argmin(np.abs(1.925 - tbAp))]
            - IaAp[np.argmin(np.abs(1.925 - taAp))]
...
def modelscore(
    ...
    if modelfeature == None:
        modelfeature = modelfeatures(modeldict, stim_start=1, stim_end=1.5, apa=apa, refreshKin=refreshKin)
