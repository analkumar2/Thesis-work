...
# tinitial = time.time()
if os.path.exists(f'../../Output/{foldernameT}/Parametersdf_o.csv'):
    Pl = pd.read_csv(f'../../Output/{foldernameT}/Parametersdf_o.csv').tail(invidx).iloc[0]
...
def update(val):
    ...
    Parameters['Na_P_changbar'] = sliders_list[16].val
    ...
    sliders_list[16].set_val(Parameters['Na_P_changbar'])
    for s in sliders_list:
...
uuuae = 0
for x in np.linspace(0.5,0.1,17):
    if uuuae%2==0:
...
sliders_list.append(Slider(axes_list[16], 'Na_P_changbar', Parameters['Na_P_changbar']*0.1, Parameters['Na_P_changbar']*10, valinit=Parameters['Na_P_changbar'], valfmt="%1.2e"))
