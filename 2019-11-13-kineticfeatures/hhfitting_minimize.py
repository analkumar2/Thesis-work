...
levelnum = 13
reader = h5py.File('../../Raw_data/Channelpedia/Kv1.1/DataKv1.1RatCHO/rCell2251.nwb')
...
bounds = Bounds([0,0.00001,0,0.00001],[1,1,1,1])
...
for level in np.arange(0.010,0.060,0.010):
    levelnum = int((level+0.060)/0.010)
    minimum = minimize(error, [1,0.005,0.05,0.050], method='Nelder-Mead', bounds=bounds)
    print(minimum.x)
    plt.plot(t,Gnorm[levelnum], color='b')
    plt.plot(t,kineticfunc_array(*minimum.x, *[0,1,1,1]), color='r')
    # plt.plot(t,kineticfunc_array(*[10e-8,1,0.0005,0.25,0.1], *[0,1,1,1]))
# plt.xlim(0,0.050)
plt.title('Activation protocol for cell 2251')
plt.xlabel('Time (s)')
